 ...
 */+ function() {"use strict";
	...
	Grid.prototype = {
		...
		initSelectRowEvent : function() {
			...
			this.gridTableBodyTable.find('tr').off('click').on('click', function(e) {
				var $this = $(this);
		...
		renderRows : function() {
			...
			for (var i = 0, j = items.length; i < j; i++) {
				...
				if (!!this.options.checked){
					checked = this.options.checked.call(this,item);
				}
				if (this.options.isShowIndexCol) {
					if(checked){
						trHtml.push('<td width="50px;"><div class="checkerbox checked" indexValue="' + i + '" data-role="indexCheckbox" data-value="' + item[this.options.identity] + '"></div></td>');
					}else{
		 ...
		 * 外部查询
		 */
		search : function(conditions,url) {           
